...
double compute_lemma_8(int p, int q, int r, double deltar, double lp, double lq, Graph &g)
{
  ...
  double cos_theta_q = (lq * lq + g.lengths[q][r] * g.lengths[q][r] - deltar * deltar) / (2 * lq * g.lengths[q][r]);
...
static int delete_edges(Graph &g)
{
  ...
  for(int h = 0; h < g.edges.size(); h++){
    ...
    for(int u = 0; u < num_points; u++){
      ...
      if(r != -1) {
        potential_points.push_back(r); i++;
      }
      else{
        break; 
      }
  ...
  for(int h = 0; h < g.edges.size(); h++){
    ...
    for(int i = 0; i < num_points; i++){    
      ...
      for(int x = 0; x < g.node_count(); x++) {
        if(is_edge(r, x, g) && are_compatible(p, q, r, x, g)) {
            compatible_points.push_back(x);
        }
      }
